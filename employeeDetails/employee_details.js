 ...
 function calculateTotalSalaries() {
  ...
  function findEmployeeById(employeeId) {
   ...
   function findSpecialization(specialization) {
      const foundEmployee = employees.find(employee => employee.specialization.toLocaleLowerCase() === specialization.toLocaleLowerCase());
      if (foundEmployee) {
      document.getElementById('employeeDetails').innerHTML =`<ps>${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.department} - ${foundEmployee.specialization} </p>`;
      }
      else{
        document.getElementById('employeeDetails').innerHTML = 'no employee has been found with that specialization';    
      } 
    } 
